...
export class MediamtxService {
  ...
  async addStreamPaths(
    ...
    await Promise.all(paths.map((p) => this.addSinglePath(p, ingestKey)));
  }
  ...
  private async addSinglePath(path: string, passphrase: string): Promise<void> {
    try {
      await axios.post(
      ...
      if (err.response?.status === 400) {
        await this.replaceSinglePath(path, passphrase);
      } else {
  ...
  private async replaceSinglePath(path: string, passphrase: string): Promise<void> {
    try {
      await axios.post(
