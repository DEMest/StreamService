...
export function StorageCard() {
  ...
  return (
    <section className="bg-surface-elevated border border-zinc-800/50 rounded-xl p-5">
      ...
      {/* Все исходы без цифр разобраны одной лестницей, а не набором
          независимых условий: ответ, не подошедший ни под одно из них (тело
          `null`, например), оставлял карточку с голым заголовком. */}
      {!isLoading && !storage && (
        isError && !data ? (
          // Красная плашка — только когда показывать нечего: при сбое фонового
          // refetch React Query отдаёт прошлые данные, и тревога над ними
          // выглядела бы так, будто устаревшие цифры актуальны.
          <p className="text-sm text-red-400">Не удалось получить метрику хранилища</p>
        ) : (
          // Тон светлее, чем у «Загрузки»: там серый уместен — процесс идёт сам
          // и вмешательства не просит. Здесь же это единственное содержимое
          // карточки, и набирать его самым тусклым тоном палитры значит
          // спрятать сообщение, ради которого карточка и осталась на экране.
          <div className="flex flex-col gap-1">
            <p className="text-sm text-zinc-400">Данные хранилища сейчас недоступны</p>
            <p className="text-[11px] text-zinc-600">
              Обновите страницу. Если не пройдёт само — сообщите администратору.
            </p>
          </div>
        )
      )}
