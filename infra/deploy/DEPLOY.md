...
Настройка разовая и **автодеплоем не выполняется**: `deploy.sh` работает только
с `docker compose`, конфиг внешнего nginx он не копирует и не перезагружает.
Пока шаги ниже не сделаны руками, мерж в `main` ничего в этой части не меняет —
админка продолжает работать по-старому на основном домене.
...
2. **Конфиг** `nginx-streamservice.conf` → на сервер, `nginx -t`, `reload`.
...
> **Порядок именно такой: сначала конфиг, потом сертификат.** Соблазн сделать
> наоборот велик, но выпустить сертификат первым нельзя: пока нет `server`-блока
> с `server_name admin.liga-live.ru`, HTTP-01 проверка уходит в default-сервер и
> проваливается. А конфиг применяется безопасно и без сертификата: оба блока
> читают один и тот же уже существующий файл, `nginx -t` проходит, основной сайт
> не замечает ничего — на поддомене будет лишь предупреждение браузера о
> несовпадении имени, и живёт оно до шага 3.
>
