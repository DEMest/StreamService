...
describe('RecordingController', () => {
  ...
  describe('serveHls path traversal (fix #4)', () => {
    it('rejects path that starts with recordingDir + suffix (no separator)', async () => {
      // Use an OS-resolved path to avoid Windows drive-letter normalization mismatch.
      ...
      // Provide a relative path that path.resolve will normalize into
      // '<recordingDir>evil/master.m3u8' — starts with recordingDir string but
      // is NOT inside the directory (no separator after recordingDir).
      const traversalRel = '../b1evil/master.m3u8';
      const res = makeRes();

      await controller.serveHls('org', 'b1', traversalRel, res);

      expect(res.status).toHaveBeenCalledWith(403);
    });

    it('allows a normal file inside recordingDir', async () => {
      const recordingDir = path.resolve('/recordings/archive/org/b1');
      mockPrisma.broadcast.findFirst.mockResolvedValue({ id: 'b1' });
      mockRecordingService.getRecordingDir.mockResolvedValue(recordingDir);
      ...
      await controller.serveHls('org', 'b1', 'master.m3u8', res);

      // No 403 traversal rejection should fire for a legit file
      const forbiddenCalls = (res.status as jest.Mock).mock.calls.filter(c => c[0] === 403);
      expect(forbiddenCalls.length).toBe(0);
    });
  });

  describe('serveLiveHls (fix #4 + #5)', () => {
    it('rejects path traversal (recordingDir + evil suffix)', async () => {
      mockPrisma.stream.findFirst.mockResolvedValue({ isPublic: true, previewKey: null });
      const liveDirResolved = path.resolve(path.join('/hls/live', 'org1'));
      const traversalRel = `../${path.basename(liveDirResolved)}evil/master.m3u8`;

      const res = makeRes();
      await controller.serveLiveHls('org1', traversalRel, undefined, res);

      expect(res.status).toHaveBeenCalledWith(403);
    });

    it('returns 404 for private stream when no key provided', async () => {
      mockPrisma.stream.findFirst.mockResolvedValue({ isPublic: false, previewKey: 'secret' });

      const res = makeRes();
      await controller.serveLiveHls('org1', 'master.m3u8', undefined, res);

      expect(res.status).toHaveBeenCalledWith(404);
    });

    it('returns 404 for private stream when wrong key provided', async () => {
      mockPrisma.stream.findFirst.mockResolvedValue({ isPublic: false, previewKey: 'secret' });

      const res = makeRes();
      await controller.serveLiveHls('org1', 'master.m3u8', 'wrong', res);

      expect(res.status).toHaveBeenCalledWith(404);
    });

    it('allows access to private stream when correct key provided', async () => {
      mockPrisma.stream.findFirst.mockResolvedValue({ isPublic: false, previewKey: 'secret' });
      spyExistsSync.mockReturnValue(true);

      const res = makeRes();
      await controller.serveLiveHls('org1', 'master.m3u8', 'secret', res);

      // Should NOT be 404 (i.e. went through to file-serving path)
    ...
    it('allows access to public stream without key', async () => {
      mockPrisma.stream.findFirst.mockResolvedValue({ isPublic: true, previewKey: null });
      spyExistsSync.mockReturnValue(true);
    ...
    it('default route uses slug="" in Stream lookup (backward-compat)', async () => {
      mockPrisma.stream.findFirst.mockResolvedValue({ isPublic: true, previewKey: null });
      spyExistsSync.mockReturnValue(true);
